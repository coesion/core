...
`tools/security-audit.allowlist.json` must include intentional findings from tool/test harness files (including test fixtures used to assert SEC001/SEC002), or CI `composer test-security` fails with scanner hits even when behavior is expected by design.
Capability Matrix drift: `docs/AUDIT.md` can lag behind implemented code capabilities; as of 2026-02-18, Query Builder (`classes/SQL.php` + `tests/SQLBuilderTest.php`), Migrations (`classes/Migration.php` + `tools/migrate.php` + `tests/MigrationTest.php`), Testing Utilities (`tests/support/AgentHttpHarness.php`), and PSR-like interop (`classes/Interop/*` + `tests/InteropTest.php`) were implemented while still marked `Missing` in the matrix.
Default router benchmark config (BENCH_ITERATIONS=5000, BENCH_REPEATS=5 across small/medium/large) is very slow in this environment mainly because laravel:large warm/cold cases take ~152-156 seconds per repeat (~12-13 minutes per mode), making full runs exceed 20 minutes.
Router benchmark harness currently gives non-equivalent dynamic matching: Core/Symfony enforce id regex constraints, while Laravel/Slim do not; additionally FastRoute setup registers /user/:id/... without converting to FastRoute {id} syntax, so many dynamic paths likely benchmark NOT_FOUND dispatch.
Benchmark guardrail discovery: with medium+ route sets, core_request mode can produce unmatched dispatches when core.route.auto_optimize=true; forcing auto_optimize=false for non-loop benchmark runs avoids false-fast 404 paths and keeps route-match parity checks passing.
Using Illuminate Router dispatch() outside full Laravel bootstrap requires container bindings for Illuminate\\Routing\\Contracts\\CallableDispatcher and ControllerDispatcher; otherwise dispatch fails with BindingResolutionException in benchmark harness.
Route::add was doing optimized_tree insertion even when core.route.loop_mode=true (where compiled dispatcher is used instead); skipping auto_optimize work in loop mode reduces route registration overhead without changing loop dispatch behavior.
Route cold-path discovery: precomputing compile-only metadata (segments/static_prefix/specificity/dynamic_bucket) in Route::__construct for every route can cause severe core_request:cold:large regressions; computing these lazily in compile-only paths restores expected behavior while preserving route parity.
Route cold-path discovery: deferring dynamic regex compilation (pattern/matcher_pattern) to first-use did not materially reduce core_request:cold:large in benchmark defaults, indicating route object construction + add-path work is the dominant cost at large scales.
Benchmark progress rendering discovery: ANSI erase sequence (`\033[2K`) is not reliably honored in some terminals (notably certain Windows/IDE shells), causing stale suffix artifacts like `00:214` or merged timestamps; use carriage-return with width-based padding instead of ANSI clear for stable ETA text.
Board-facing markdown in this repo should align to plans/08-agentic-master-execution.md for decision-board structure and docs/guides/Proof-Weekly-Template.md for claim/verification traceability placeholders.
Modernizing callback signatures in `classes/CLI.php` can leave a stale `phpstan-baseline.neon` ignore for `preg_replace_callback` callback type; that stale ignore itself fails `composer test-all` and must be removed.

...
- `Work::run()` round-robins tasks until complete.
- `Work::after()` installs shutdown callbacks for deferred work.

Public API:
- `Work::add($id, $job = null)` adds a task (or auto-assigns ID).
- `Work::send($id, $passValue)` sends a value into a running task.
- `Work::run()` runs the pool.
- `Work::after(callable $callback)` defers execution to shutdown.

Supporting class:
- `TaskCoroutine` wraps a generator with `run()`, `pass()`, `complete()`.
...
The [Work](./Work.md) module allow you to execute multiple jobs in parallel, via [cooperative multitasking](http://en.wikipedia.org/wiki/Cooperative_multitasking).

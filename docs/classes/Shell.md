...
- `Shell::pipe(...$items)` builds a piped command.
- `Shell::sequence(...$items)` builds a logical AND sequence.
- `Shell::alias($command, callable $callback)` defines an alias.
- `Shell::escape($arg)` escapes shell args.
- `Shell::run()` executes and returns output.

Example:
```php
$out = Shell::ls(['a' => true])->run();
```
...
The [Shell](./Shell.md) class can be used to execute OS commands.

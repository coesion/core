...
- Auto delimiter is supported for reading.
- The first row is treated as a header in read mode.

Public API:
- `CSV::open($file, $format = CSV::AUTO)` opens for read.
- `CSV::create($file, $format = CSV::STANDARD)` opens for write.
- `CSV::fromSQL($sql, $format = CSV::AUTO)` creates CSV from a SQL query.
- `CSV::fromTable($table, $format = CSV::AUTO)` creates CSV from array data.
- `CSV::SQL($sql)` writes rows from SQL query.
- `CSV::write($row)` writes a row, using the schema.
- `CSV::read()` yields rows as arrays.
- `CSV::each(?callable $looper = null)` iterates or returns all rows.
- `CSV::convert($filename, $format = CSV::STANDARD)` converts to another format.
- `CSV::schema($schema = null)` sets or gets the header schema.
- `CSV::asString()` returns the file contents.

Example:
```php
$csv = CSV::create('out.csv');
$csv->write(['id' => 1, 'name' => 'Ada']);
$csv->write(['id' => 2, 'name' => 'Grace']);
```
...
The [CSV](./CSV.md) module allow you to read and write Comma Separated Value data.

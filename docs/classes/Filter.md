...
- `Filter::single($name, callable $modder)` replaces filters.
- `Filter::remove($name, callable $modder = null)` removes filters.
- `Filter::with($name, $default, ...$args)` applies filters.
...
The [Filter](./Filter.md) module allow you to permit user overrides to certain values.

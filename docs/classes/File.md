...
- `File::__callStatic` forwards to the adapter of the located mount.

Public API:
- `File::mount($alias, $driver, $options = null)` registers a filesystem.
- `File::unmount($alias)` removes a mount.
- `File::mounts()` lists aliases.
- `File::locate($path)` resolves a URI to `[mount, path]`.
- `File::resolvePath($path)` normalizes path segments.
- `File::search($pattern, $recursive = true)` searches all mounts.
- `File::move($old, $new)` moves files across mounts.
...
The [File](./File.md) module expose utilities for working with virtual filesystems.

...
- `Event::trigger($name, ...$args)` dispatches.
- `Event::single($name, callable $listener)` is a convenience for `onSingle`.
...
The [Event](./Event.md) module allow you to bind callbacks to custom events.

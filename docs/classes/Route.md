...
- `Route::get($pattern, $callback = null)` GET route.
- `Route::post($pattern, $callback = null)` POST route.
- `Route::any($pattern, $callback = null)` any method.
- `Route::map($pattern, $callbacks = [])` per-method map.
- `->with($callback)` sets callback.
- `->before($callback)` adds middleware.
- `->after($callback)` adds middleware.
- `->via(...$methods)` sets allowed methods.
- `->rules(array $rules)` sets regex for params.
- `->tag($name)` names a route.
- `->getURL($params = [])` builds a URL from a tagged route.
- `Route::URL($name, $params = [])` returns a `URL` object.
...
The syntax is the same as for the `Response::push` [method](./Response.md#http2-server-push).
...
The [Route](./Route.md) module allow you to bind callbacks to HTTP requests.
...
> **Note:** The returned value of the `Route::URL` method is an [URL](./URL.md) object.

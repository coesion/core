...
class SQL {
  ...
  /**
   * Check if a datasource connection is registered.
   * @param string $name The datasource name
   * @return bool
   */
...
class SQLConnection {
  ...
  public function __construct($dsn, $username=null, $password=null, $options=[]){
    ...
    if ($this->is_mysql && defined('PDO::MYSQL_ATTR_MULTI_STATEMENTS')) {
      $base_options[PDO::MYSQL_ATTR_MULTI_STATEMENTS] = true;
    }
    $this->connection = [
      'dsn'        => $dsn,
      'pdo'        => null,
    ...
    // SQLite doesn't support some driver-specific options; keep base defaults.
  }
  ...
  public function exec($query, $params=[], $pdo_params=[]){
    ...
    if($statement = $this->prepare($query, $pdo_params)){
      ...
      Event::trigger('core.log', 'sql.error', ['message' => $error[2], 'query' => $query, 'params' => $params]);
      return false;
    }
  ...
  public function reduce($query, $params=[], $looper = null, $initial = null){
    ...
    if(( $res = $this->exec($query,$params, $this->bufferedQueryParams()) ) && is_callable($looper) ){
  ...
  public function each($query, $params=[], ?callable $looper = null){
    ...
    if( $res = $this->exec($query,$params, $this->bufferedQueryParams()) ){
  ...
  private function bufferedQueryParams(): array {
    return ($this->is_mysql && defined('PDO::MYSQL_ATTR_USE_BUFFERED_QUERY'))

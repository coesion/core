...
trait Persistence {
  ...
  public static function persistenceOptions($options=null){
    ...
    if (!isset($_options[$class])) {
      $_options[$class] = ['table'=>null,'key'=>'id'];
    }
    $opts =& $_options[$class];
    if ($options === null) {
      if (empty($opts['table'])) {
        $defaults = static::persistenceDefaultOptionsFor($class);
        $opts['table'] = $defaults['table'];
        if (empty($opts['key'])) $opts['key'] = $defaults['key'];
      }
      return $opts;
    }
    ...
    if (is_array($options)) {
      ...
      return isset($opts[$options]) ? $opts[$options] : '';
    }
  }

  private static function persistenceDefaultOptionsFor($class){
    if (defined("$class::_PRIMARY_KEY_")){
    ...
    switch (substr($s = strtolower($class), -1)) {

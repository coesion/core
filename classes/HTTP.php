...
class HTTP {
  ...
  protected static function request($method, $url, $data=[], array $headers=[], $data_as_json=false, $username=null, $password = null){
    ...
    $opt = [
      ...
      CURLOPT_SSL_VERIFYHOST  => static::$verify_ssl ? 2 : 0,
      CURLOPT_SSL_VERIFYPEER  => static::$verify_ssl ? true : false,
    ...
    Event::trigger("core.log", "http.request", ["url" => $url, "method" => $http_method, "info" => static::$last_info]);
  ...
  public static function verify($value=null){
    return $value===null ? static::$verify_ssl : static::$verify_ssl = (bool)$value;
  }

  public static function timeout($seconds=null){
    if ($seconds === null) return static::$timeout;
    $seconds = (int)$seconds;
    if ($seconds < 1) $seconds = 1;
    return static::$timeout = $seconds;
  }
